...
class UNePyNoExportType_Box2D : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool Equals(const FBox2D& Other, double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	double ComputeSquaredDistanceToPoint(const FVector2D& Point);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "ExpandBy", OriginalName = "ExpandBy"))
	FBox2D ExpandBy_Overload1(double W);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "ExpandBy", OriginalName = "ExpandBy"))
	FBox2D ExpandBy_Overload2(const FVector2D& V);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	FBox2D Overlap(const FBox2D& Other);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	bool IsInsideOrOn(const FVector2D& TestPoint);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	FBox2D MoveTo(const FVector2D& Destination);
...
class UNePyNoExportType_BoxSphereBounds : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FBoxSphereBounds", OriginalName = "FBoxSphereBounds"))
	void FBoxSphereBounds_Overload2(const FVector& InOrigin, const FVector& InBoxExtent, double InSphereRadius);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	double ComputeSquaredDistanceFromBoxToPoint(const FVector& Point);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	static bool SpheresIntersect(const FBoxSphereBounds& A, const FBoxSphereBounds& B, double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	FBoxSphereBounds ExpandBy(double ExpandAmount);
...
class UNePyNoExportType_Color : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static float DequantizeUNorm8ToFloat(int32 Value8);

	UFUNCTION(meta=(bFriendFunction = false))
	static float DequantizeUNorm16ToFloat(int32 Value16);

	UFUNCTION(meta=(bFriendFunction = false))
	static uint8 Requantize10to8(int32 Value10);

	UFUNCTION(meta=(bFriendFunction = false))
	static uint8 Requantize16to8(int32 Value16);
	...
	UFUNCTION(meta=(bFriendFunction = true))
	uint32 GetTypeHash(const FColor& InSelf);
...
class UNePyNoExportType_Guid : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static FGuid Combine(const FGuid& GuidA, const FGuid& GuidB);
...
class UNePyNoExportType_LinearColor : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = true))
	uint32 GetTypeHash(const FLinearColor& InSelf);
...
class UNePyNoExportType_Math : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "RandRange", OriginalName = "RandRange"))
	static double RandRange_Overload3(double InMin, double InMax);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "IsNearlyEqual", OriginalName = "IsNearlyEqual", CPP_Default_ErrorTolerance = "UE_SMALL_NUMBER"))
	static bool IsNearlyEqual_Overload1(float A, float B, float ErrorTolerance);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "IsNearlyZero", OriginalName = "IsNearlyZero", CPP_Default_ErrorTolerance = "UE_SMALL_NUMBER"))
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static int32 Max3Index(double A, double B, double C);

	UFUNCTION(meta=(bFriendFunction = false))
	static int32 Min3Index(double A, double B, double C);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "Clamp", OriginalName = "Clamp"))
	static double Clamp_Overload1(double X, double MinValue, double MaxValue);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "Clamp", OriginalName = "Clamp"))
	static float Clamp_Overload2(float X, float Min, float Max);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "Clamp", OriginalName = "Clamp"))
	static double Clamp_Overload3(double X, double Min, double Max);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "Clamp", OriginalName = "Clamp"))
	static int64 Clamp_Overload4(int64 X, int32 Min, int32 Max);

	UFUNCTION(meta=(bFriendFunction = false))
	static double Wrap(double X, double Min, double Max);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static double FInterpConstantTo(double Current, double Target, double DeltaTime, double InterpSpeed);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static double InvExpApprox(double X);

	UFUNCTION(meta=(bFriendFunction = false))
	static FVector RayPlaneIntersection(const FVector& RayOrigin, const FVector& RayDirection, const FPlane& Plane);

	UFUNCTION(meta=(bFriendFunction = false))
	static double RayPlaneIntersectionParam(const FVector& RayOrigin, const FVector& RayDirection, const FPlane& Plane);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "TruncateToHalfIfClose", OriginalName = "TruncateToHalfIfClose", CPP_Default_Tolerance = "UE_SMALL_NUMBER"))
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "GetBaryCentric2D", OriginalName = "GetBaryCentric2D"))
	static FVector GetBaryCentric2D_Overload1(const FVector& Point, const FVector& A, const FVector& B, const FVector& C);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "GetBaryCentric2D", OriginalName = "GetBaryCentric2D"))
	static FVector GetBaryCentric2D_Overload2(const FVector2D& Point, const FVector2D& A, const FVector2D& B, const FVector2D& C);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static double WeightedMovingAverage(double CurrentSample, double PreviousSample, double Weight);

	UFUNCTION(meta=(bFriendFunction = false))
	static double DynamicWeightedMovingAverage(double CurrentSample, double PreviousSample, double MaxDistance, double MinWeight, double MaxWeight);
...
class UNePyNoExportType_Matrix : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__mul__", OriginalName = "operator*"))
	FMatrix __mul___Overload2(double Other);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__imul__", OriginalName = "operator*="))
	void __imul___Overload2(double Other);

	UFUNCTION(meta=(bFriendFunction = false, OriginalName = "operator=="))
	bool __eq__(const FMatrix& Other);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool Equals(const FMatrix& Other, double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_SMALL_NUMBER"))
	void RemoveScaling(double Tolerance);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_SMALL_NUMBER"))
	FMatrix GetMatrixWithoutScale(double Tolerance);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_SMALL_NUMBER"))
	FVector ExtractScaling(double Tolerance);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_SMALL_NUMBER"))
	FVector GetScaleVector(double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	double GetMinimumAxisScale();

	UFUNCTION(meta=(bFriendFunction = false))
	double GetMaximumAxisScale();

	UFUNCTION(meta=(bFriendFunction = false))
	FMatrix ApplyScale(double Scale);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	bool GetFrustumNearPlane(FPlane& OuTPln);

	UFUNCTION(meta=(bFriendFunction = false))
	bool GetFrustumFarPlane(FPlane& OuTPln);

	UFUNCTION(meta=(bFriendFunction = false))
	bool GetFrustumLeftPlane(FPlane& OuTPln);

	UFUNCTION(meta=(bFriendFunction = false))
	bool GetFrustumRightPlane(FPlane& OuTPln);

	UFUNCTION(meta=(bFriendFunction = false))
	bool GetFrustumTopPlane(FPlane& OuTPln);

	UFUNCTION(meta=(bFriendFunction = false))
	bool GetFrustumBottomPlane(FPlane& OuTPln);
...
class UNePyNoExportType_Vector : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FVector", OriginalName = "FVector"))
	void FVector_Overload2(double InF);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FVector", OriginalName = "FVector"))
	void FVector_Overload4(double InX, double InY, double InZ);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FVector", OriginalName = "FVector"))
	void FVector_Overload5(int32 InVector);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FVector", OriginalName = "FVector"))
	void FVector_Overload6(int32 A);

	UFUNCTION(meta=(bFriendFunction = false))
	FVector Cross(const FVector& V2);

	UFUNCTION(meta=(bFriendFunction = false))
	static FVector CrossProduct(const FVector& A, const FVector& B);

	UFUNCTION(meta=(bFriendFunction = false))
	double Dot(const FVector& V);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__sub__", OriginalName = "operator-"))
	FVector __sub___Overload2(double Bias);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__add__", OriginalName = "operator+"))
	FVector __add___Overload2(double Bias);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__mul__", OriginalName = "operator*"))
	FVector __mul___Overload1(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__div__", OriginalName = "operator/"))
	FVector __div___Overload1(double Scale);
	...
	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool Equals(const FVector& V, double Tolerance);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool AllComponentsEqual(double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__imul__", OriginalName = "operator*="))
	FVector __imul___Overload1(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__idiv__", OriginalName = "operator/="))
	FVector __idiv___Overload1(double Scale);
	...
	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool IsNearlyZero(double Tolerance);

	UFUNCTION(meta=(bFriendFunction = false))
	bool IsZero();

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_LengthSquaredTolerance = "UE_KINDA_SMALL_NUMBER"))
	bool IsUnit(double LengthSquaredTolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_SMALL_NUMBER", CPP_Default_ResultIfZero = "ZeroVector"))
	FVector GetSafeNormal(double Tolerance, const FVector& ResultIfZero);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_SMALL_NUMBER", CPP_Default_ResultIfZero = "ZeroVector"))
	FVector GetSafeNormal2D(double Tolerance, const FVector& ResultIfZero);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	FVector BoundToCube(double Radius);

	UFUNCTION(meta=(bFriendFunction = false))
	FVector BoundToBox(const FVector& Min, const FVector& Max);

	UFUNCTION(meta=(bFriendFunction = false))
	FVector GetClampedToSize(double Min, double Max);

	UFUNCTION(meta=(bFriendFunction = false))
	FVector GetClampedToSize2D(double Min, double Max);

	UFUNCTION(meta=(bFriendFunction = false))
	FVector GetClampedToMaxSize(double MaxSize);

	UFUNCTION(meta=(bFriendFunction = false))
	FVector GetClampedToMaxSize2D(double MaxSize);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Radius = "MAX_int16"))
	void AddBounded(const FVector& V, double Radius);

	UFUNCTION(meta=(bFriendFunction = false))
	FVector Reciprocal();

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool IsUniform(double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	FVector RotateAngleAxis(double AngleDeg, const FVector& Axis);

	UFUNCTION(meta=(bFriendFunction = false))
	FVector RotateAngleAxisRad(double AngleRad, const FVector& Axis);

	UFUNCTION(meta=(bFriendFunction = false))
	double CosineAngle2D(FVector B);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static FVector SlerpVectorToDirection(FVector& V, FVector& Direction, double Alpha);

	UFUNCTION(meta=(bFriendFunction = false))
	static FVector SlerpNormals(FVector& NormalA, FVector& NormalB, double Alpha);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static bool PointsAreNear(const FVector& Point1, const FVector& Point2, double Dist);

	UFUNCTION(meta=(bFriendFunction = false))
	static double PointPlaneDist(const FVector& Point, const FVector& PlaneBase, const FVector& PlaneNormal);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static double DistXY(const FVector& V1, const FVector& V2);

	UFUNCTION(meta=(bFriendFunction = false))
	static double Dist2D(const FVector& V1, const FVector& V2);

	UFUNCTION(meta=(bFriendFunction = false))
	static double DistSquared(const FVector& V1, const FVector& V2);

	UFUNCTION(meta=(bFriendFunction = false))
	static double DistSquaredXY(const FVector& V1, const FVector& V2);

	UFUNCTION(meta=(bFriendFunction = false))
	static double DistSquared2D(const FVector& V1, const FVector& V2);

	UFUNCTION(meta=(bFriendFunction = false))
	static double BoxPushOut(const FVector& Normal, const FVector& Size);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static FVector Min3(const FVector& A, const FVector& B, const FVector& C);

	UFUNCTION(meta=(bFriendFunction = false))
	static FVector Max3(const FVector& A, const FVector& B, const FVector& C);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_ParallelCosineThreshold = "UE_THRESH_NORMALS_ARE_PARALLEL"))
	static bool Parallel(const FVector& Normal1, const FVector& Normal2, double ParallelCosineThreshold);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_ParallelCosineThreshold = "UE_THRESH_NORMALS_ARE_PARALLEL"))
	static bool Coincident(const FVector& Normal1, const FVector& Normal2, double ParallelCosineThreshold);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_OrthogonalCosineThreshold = "UE_THRESH_NORMALS_ARE_ORTHOGONAL"))
	static bool Orthogonal(const FVector& Normal1, const FVector& Normal2, double OrthogonalCosineThreshold);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_ParallelCosineThreshold = "UE_THRESH_NORMALS_ARE_PARALLEL"))
	static bool Coplanar(const FVector& Base1, const FVector& Normal1, const FVector& Base2, const FVector& Normal2, double ParallelCosineThreshold);
...
class UNePyNoExportType_Plane : public UNePyNoExportType_Vector
{
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FPlane", OriginalName = "FPlane"))
	void FPlane_Overload3(double InX, double InY, double InZ, double InW);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FPlane", OriginalName = "FPlane"))
	void FPlane_Overload4(FVector InNormal, double InW);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	double PlaneDot(const FVector& P);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "Normalize", OriginalName = "Normalize", CPP_Default_Tolerance = "UE_SMALL_NUMBER"))
	bool Plane_Normalize(double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	FPlane TransformByUsingAdjointT(const FMatrix& M, double DetM, const FMatrix& TA);

	UFUNCTION(meta=(bFriendFunction = false))
	FPlane TranslateBy(const FVector& V);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "Equals", OriginalName = "Equals", CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool Plane_Equals(const FPlane& V, double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__div__", OriginalName = "operator/"))
	FPlane Plane___div__(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__mul__", OriginalName = "operator*"))
	FPlane Plane___mul___Overload1(double Scale);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__imul__", OriginalName = "operator*="))
	FPlane Plane___imul___Overload1(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__imul__", OriginalName = "operator*="))
	FPlane Plane___imul___Overload2(const FPlane& V);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__idiv__", OriginalName = "operator/="))
	FPlane Plane___idiv__(double V);
...
class UNePyNoExportType_PrimaryAssetType : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = false))
	bool LexicalLess(const FPrimaryAssetType& Other);

	UFUNCTION(meta=(bFriendFunction = false))
	bool FastLess(const FPrimaryAssetType& Other);
...
class UNePyNoExportType_Quat : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FQuat", OriginalName = "FQuat"))
	void FQuat_Overload2(double InX, double InY, double InZ, double InW);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FQuat", OriginalName = "FQuat"))
	void FQuat_Overload3(const FRotator& R);

	UFUNCTION(meta=(bFriendFunction = false))
	static FQuat MakeFromRotator(const FRotator& R);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FQuat", OriginalName = "FQuat"))
	void FQuat_Overload4(const FMatrix& M);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FQuat", OriginalName = "FQuat"))
	void FQuat_Overload5(FVector Axis, double AngleRad);
	...
	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool Equals(const FQuat& Q, double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__imul__", OriginalName = "operator*="))
	FQuat __imul___Overload2(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__mul__", OriginalName = "operator*"))
	FQuat __mul___Overload4(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, OriginalName = "operator/="))
	FQuat __idiv__(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, OriginalName = "operator/"))
	FQuat __div__(double Scale);
	...
	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_SMALL_NUMBER"))
	FQuat GetNormalized(double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	double GetTwistAngle(const FVector& TwistAxis);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	double AngularDistance(const FQuat& Q);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static double Error(const FQuat& Q1, const FQuat& Q2);

	UFUNCTION(meta=(bFriendFunction = false))
	static double ErrorAutoNormalize(const FQuat& A, const FQuat& B);

	UFUNCTION(meta=(bFriendFunction = false))
	static FQuat FastLerp(const FQuat& A, const FQuat& B, double Alpha);

	UFUNCTION(meta=(bFriendFunction = false))
	static FQuat FastBilerp(const FQuat& P00, const FQuat& P10, const FQuat& P01, const FQuat& P11, double FracX, double FracY);

	UFUNCTION(meta=(bFriendFunction = false))
	static FQuat Slerp_NotNormalized(const FQuat& Quat1, const FQuat& Quat2, double Slerp);

	UFUNCTION(meta=(bFriendFunction = false))
	static FQuat Slerp(const FQuat& Quat1, const FQuat& Quat2, double Slerp);

	UFUNCTION(meta=(bFriendFunction = false))
	static FQuat SlerpFullPath_NotNormalized(const FQuat& quat1, const FQuat& quat2, double Alpha);

	UFUNCTION(meta=(bFriendFunction = false))
	static FQuat SlerpFullPath(const FQuat& quat1, const FQuat& quat2, double Alpha);

	UFUNCTION(meta=(bFriendFunction = false))
	static FQuat Squad(const FQuat& quat1, const FQuat& tang1, const FQuat& quat2, const FQuat& tang2, double Alpha);

	UFUNCTION(meta=(bFriendFunction = false))
	static FQuat SquadFullPath(const FQuat& quat1, const FQuat& tang1, const FQuat& quat2, const FQuat& tang2, double Alpha);

	UFUNCTION(meta=(bFriendFunction = false))
	static void CalcTangents(const FQuat& PrevP, const FQuat& P, const FQuat& NextP, double Tension, FQuat& OutTan);
...
class UNePyNoExportType_Rotator : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FRotator", OriginalName = "FRotator"))
	void FRotator_Overload2(double InF);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FRotator", OriginalName = "FRotator"))
	void FRotator_Overload3(double InPitch, double InYaw, double InRoll);
	...
	UFUNCTION(meta=(bFriendFunction = false, OriginalName = "operator*"))
	FRotator __mul__(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, OriginalName = "operator*="))
	FRotator __imul__(double Scale);
	...
	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool Equals(const FRotator& R, double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	double GetManhattanDistance(const FRotator& Rotator);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static uint8 CompressAxisToByte(double Angle);

	UFUNCTION(meta=(bFriendFunction = false))
	static double DecompressAxisFromByte(uint8 Angle);

	UFUNCTION(meta=(bFriendFunction = false))
	static uint16 CompressAxisToShort(double Angle);

	UFUNCTION(meta=(bFriendFunction = false))
	static double DecompressAxisFromShort(uint16 Angle);
...
class UNePyNoExportType_SlateColor : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FSlateColor", OriginalName = "FSlateColor"))
	void FSlateColor_Overload3(FColor InColor);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FSlateColor", OriginalName = "FSlateColor"))
	void FSlateColor_Overload4(EStyleColor InColorTableId);
...
class UNePyNoExportType_SoftObjectPath : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FSoftObjectPath", OriginalName = "FSoftObjectPath"))
	void FSoftObjectPath_Overload4(FTopLevelAssetPath InAssetPath, FString InSubPathString);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FSoftObjectPath", OriginalName = "FSoftObjectPath"))
	void FSoftObjectPath_Overload5(FName InPackageName, FName InAssetName, FString InSubPathString);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FSoftObjectPath", OriginalName = "FSoftObjectPath"))
	void FSoftObjectPath_Overload6(FTopLevelAssetPath InAssetPath);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FSoftObjectPath", OriginalName = "FSoftObjectPath"))
	void FSoftObjectPath_Overload7(FName InAssetPathName, FString InSubPathString);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FSoftObjectPath", OriginalName = "FSoftObjectPath"))
	void FSoftObjectPath_Overload9(const UObject* InObject);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	void AppendString(FString& Builder);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	void SetSubPathString(const FString& InSubPathString);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "SetPath", OriginalName = "SetPath", CPP_Default_InSubPathString = "FString()"))
	void SetPath_Overload1(const FTopLevelAssetPath& InAssetPath, FString InSubPathString);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "SetPath", OriginalName = "SetPath"))
	void SetPath_Overload2(const FString& Path);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	bool LexicalLess(const FSoftObjectPath& Other);

	UFUNCTION(meta=(bFriendFunction = false))
	bool FastLess(const FSoftObjectPath& Other);
...
class UNePyNoExportType_Transform : public UObject
{
	...
	FTransform GetScaled_Overload1(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "GetScaled", OriginalName = "GetScaled"))
	FTransform GetScaled_Overload2(FVector Scale);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_SMALL_NUMBER"))
	static FVector GetSafeScaleReciprocal(const FVector& InScale, double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	static bool AreRotationsEqual(const FTransform& A, const FTransform& B, double Tolerance);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	static bool AreTranslationsEqual(const FTransform& A, const FTransform& B, double Tolerance);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	static bool AreScale3DsEqual(const FTransform& A, const FTransform& B, double Tolerance);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool RotationEquals(const FTransform& Other, double Tolerance);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool TranslationEquals(const FTransform& Other, double Tolerance);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool Scale3DEquals(const FTransform& Other, double Tolerance);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool Equals(const FTransform& Other, double Tolerance);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool EqualsNoScale(const FTransform& Other, double Tolerance);
...
class UNePyNoExportType_Vector2D : public UObject
{
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static FVector2D UnitX();

	UFUNCTION(meta=(bFriendFunction = false))
	static FVector2D UnitY();

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FVector2D", OriginalName = "FVector2D"))
	void FVector2D_Overload1();

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FVector2D", OriginalName = "FVector2D"))
	void FVector2D_Overload2(double InX, double InY);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FVector2D", OriginalName = "FVector2D"))
	void FVector2D_Overload3(double InF);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FVector2D", OriginalName = "FVector2D"))
	void FVector2D_Overload4(int32 InPos);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__mul__", OriginalName = "operator*"))
	FVector2D __mul___Overload1(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__div__", OriginalName = "operator/"))
	FVector2D __div___Overload1(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__add__", OriginalName = "operator+"))
	FVector2D __add___Overload2(double A);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__sub__", OriginalName = "operator-"))
	FVector2D __sub___Overload2(double A);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	bool ComponentwiseAllLessThan(const FVector2D& Other);

	UFUNCTION(meta=(bFriendFunction = false))
	bool ComponentwiseAllGreaterThan(const FVector2D& Other);

	UFUNCTION(meta=(bFriendFunction = false))
	bool ComponentwiseAllLessOrEqual(const FVector2D& Other);

	UFUNCTION(meta=(bFriendFunction = false))
	bool ComponentwiseAllGreaterOrEqual(const FVector2D& Other);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__imul__", OriginalName = "operator*="))
	FVector2D __imul___Overload1(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__idiv__", OriginalName = "operator/="))
	FVector2D __idiv___Overload1(double V);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	static double DistSquared(const FVector2D& V1, const FVector2D& V2);

	UFUNCTION(meta=(bFriendFunction = false))
	static double Distance(const FVector2D& V1, const FVector2D& V2);

	UFUNCTION(meta=(bFriendFunction = false))
	static double CrossProduct(const FVector2D& A, const FVector2D& B);
	...
	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool Equals(const FVector2D& V, double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	double Dot(const FVector2D& V2);

	UFUNCTION(meta=(bFriendFunction = false))
	FVector2D GetRotated(double AngleDeg);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_SMALL_NUMBER"))
	FVector2D GetSafeNormal(double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false))
	FVector2D ClampAxes(double MinAxisVal, double MaxAxisVal);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "FVector2D", OriginalName = "FVector2D"))
	void FVector2D_Overload5(const FVector2D& From);
...
class UNePyNoExportType_Vector4 : public UObject
{
	...
	void FVector4(double InX, double InY, double InZ, double InW);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__mul__", OriginalName = "operator*"))
	FVector4 __mul___Overload1(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__div__", OriginalName = "operator/"))
	FVector4 __div___Overload1(double Scale);
	...
	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__idiv__", OriginalName = "operator/="))
	FVector4 __idiv___Overload1(const FVector4& V);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__imul__", OriginalName = "operator*="))
	FVector4 __imul___Overload2(double Scale);

	UFUNCTION(meta=(bFriendFunction = false, ScriptName = "__idiv__", OriginalName = "operator/="))
	FVector4 __idiv___Overload2(double Scale);
	...
	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool Equals(const FVector4& V, double Tolerance);

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_LengthSquaredTolerance = "UE_KINDA_SMALL_NUMBER"))
	bool IsUnit3(double LengthSquaredTolerance);

	UFUNCTION(meta=(bFriendFunction = false))
	FString ToString();

	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_SMALL_NUMBER"))
	FVector4 GetSafeNormal(double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = false, CPP_Default_Tolerance = "UE_KINDA_SMALL_NUMBER"))
	bool IsNearlyZero3(double Tolerance);
	...
	UFUNCTION(meta=(bFriendFunction = true))
	uint32 GetTypeHash(const FVector4& InSelf);
