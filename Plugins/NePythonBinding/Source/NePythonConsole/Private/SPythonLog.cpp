...
#define LOCTEXT_NAMESPACE "NePythonConsole"

namespace SPythonLogUtils
{
	static FString Filename = FPaths::Combine(*FPaths::ProjectLogDir(), *FString::Printf(TEXT("%s%s"), TEXT("pycmd"), TEXT(".txt")));
	static const int MaxCmdLength = 200;

	void Serialize(const TArray<FString>& ArrayData, FString& Result)
	{
		int End = ArrayData.Num();
		int Start = std::max(End - MaxCmdLength, 0);

		for (; Start < End; ++Start)
		{
			Result.Append(ArrayData[Start]);
			Result.Append("\n");
		}
	}

	void Deserialize(FString& FileData, TArray<FString>& ArrayData)
	{
		if (FileData.IsEmpty())
			return;
		...
		RightStr = FileData;
		while (RightStr.Split("\n", &LeftStr, &RightStr))
		{
			ArrayData.Add(LeftStr);
		}
	}

	void LoadCmds(TArray<FString>& ArrayData)
	{
		FString LoadStr;
		if (!FFileHelper::LoadFileToString(LoadStr, *Filename))
		{
			UE_LOG(LogTemp, Error, TEXT("Failed to load pycmd file: %s"), *Filename);
		}
		Deserialize(LoadStr, ArrayData);
	}

	void SaveCmds(const TArray<FString>& ArrayData)
	{
		FString SaveStr;
		Serialize(ArrayData, SaveStr);
		if (!FFileHelper::SaveStringToFile(SaveStr, *Filename, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM))
		{
			UE_LOG(LogTemp, Error, TEXT("Failed to write to pycmd file: %s"), *Filename);
		}
	}
}
...
void SPythonConsoleInputBox::Construct(const FArguments& InArgs)
{
	...
	SPythonLogUtils::LoadCmds(History);
...
void SPythonConsoleInputBox::OnTextCommitted(const FText& InText, ETextCommit::Type CommitInfo)
{
	if (CommitInfo == ETextCommit::OnEnter)
	{
		if (!InText.IsEmpty())
		{
			...
			int nOrgSize = History.Num();
			if (nOrgSize > 0)
			{
				if (History[nOrgSize - 1] != ExecString)
				{
					History.Add(ExecString);
					SPythonLogUtils::SaveCmds(History);
				}
			}
			else
			{
...
bool FPythonLogTextLayoutMarshaller::AppendPendingMessage(const TCHAR* InText, const ELogVerbosity::Type InVerbosity, const FName& InCategory)
{
	return SPythonLog::CreateLogMessages(InText, InVerbosity, InCategory, Messages);
}
...
void FPythonLogTextLayoutMarshaller::AppendPendingMessagesToTextLayout()
{
	...
	{
		const TSharedPtr<FLogMessage> Message = Messages[MessageIndex];
...
bool FPythonLogTextLayoutMarshaller::SubmitPendingMessages()
{
...
void SPythonLog::RequestForceScroll(bool bIfUserHasNotScrolledUp)
{
