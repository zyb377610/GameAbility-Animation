...
1. **参数语义不明 → 先查引擎源码**，不凭直觉猜测。路径: `D:\UE5\UE_5.6\Engine\Source\`，grep 定位函数实现后逐行确认。
2. **`-1` 不是万能值**。UE 源码中 `-1` 可能参与除法、被 Clamp 钳制、或代表 INDEX_NONE。不要假设它等于"无限/全部"。
3. **改一个值 → 验证 → 再改下一个**。不批量修改未经验证的参数。
